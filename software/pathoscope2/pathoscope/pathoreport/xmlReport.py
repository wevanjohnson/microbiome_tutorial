...
class Organism:
	...
	def buildElement(self):
		...
		if self.reads:
			...
			for sequence in self.reads:
				sequenceElem = ET.SubElement(readsElem, "sequence")
				sequenceElem.text = sequence
		return root
...
def writePathoXML(run_parameters, outputFileName, h_annoT, h_ti_contig, 
		h_refRead, h_refScore, h_gisPerTi, h_tiRef, reads, samFile, mySqlConf):
...
def buildOrganismsElement(h_annoT, h_ti_contig, hostTaxon, h_refRead, h_refScore, 
		h_gisPerTi, h_tiRef, reads, samFile, mySqlConf):
	...
	for ti in h_gisPerTi:
		...
		organism.reads = h_refRead.get(refId,None)
